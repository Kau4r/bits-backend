...
const createBorrowing = async (req, res) => {
    ...
    if (itemType) {
        ...
        return res.status(201).json({
            success: true,
            data: { message: 'Borrow request submitted. Awaiting Lab Tech approval.', borrowing }
        });
    }

    // LEGACY: Specific item(s) request (for backward compatibility)
    if (!items || !Array.isArray(items) || items.length === 0) {
        return res.status(400).json({ success: false, error: 'Either itemType or items array is required' });
    }
    ...
    if (borrowings.length === 0 && errors.length > 0) {
        return res.status(400).json({ success: false, error: 'Failed to create borrow requests', meta: { details: errors } });
    }
...
const approveBorrowing = async (req, res) => {
    ...
    if (borrowing.Status !== 'PENDING') {
        return res.status(400).json({
            success: false, error: `Cannot approve. Current status is ${borrowing.Status}`
        });
    }
    ...
    if (!itemId) {
        if (!assignedItemId) {
            return res.status(400).json({
                success: false, error: 'Please select a specific item to assign to this request'
            });
        }
    ...
    if (item.Status !== 'AVAILABLE') {
        return res.status(409).json({
            success: false, error: `Item ${item.Item_Code || item.Item_Type} is no longer available`
        });
    }
    ...
    res.json({
        success: true,
        data: { message: `Approved borrow request for ${itemName}`, borrowing: updatedBorrowing }
    });
};
...
const rejectBorrowing = async (req, res) => {
    ...
    if (borrowing.Status !== 'PENDING') {
        return res.status(400).json({
            success: false, error: `Cannot reject. Current status is ${borrowing.Status}`
        });
    }
    ...
    res.json({
        success: true,
        data: { message: `Rejected borrow request for ${itemName}`, borrowing: updatedBorrowing }
    });
};
...
const returnBorrowing = async (req, res) => {
    ...
    if (borrowing.Status !== 'BORROWED' && borrowing.Status !== 'OVERDUE') {
        return res.status(400).json({
            success: false, error: `Cannot return. Current status is ${borrowing.Status}`
        });
    }
    ...
    res.json({
        success: true,
        data: { message: `Item ${itemName} returned successfully` }
    });
};
